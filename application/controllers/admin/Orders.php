...
class Orders extends CI_Controller
{
    ...
    public $tbl_name = "orders";
    public $tbl_orders_cards = "orders_cards";
    public $tbl_orders_shippings = "orders_shippings";
                ...
                if ($insert_id > 0)
                {
                    if(!empty($card_selected))
                    {
                        foreach($card_selected as $card_id)
                        {
                            ...
                            setInsertData($this->tbl_orders_cards, $dataCardArr);
                        }
                    }
                    ...
                    if(!empty($shipment_selected))
                    {
                        foreach($shipment_selected as $card_type)
                        {
                            ...
                            setInsertData($this->tbl_orders_shippings, $dataShippingArr);
                        }
                    }
    ...
    public function edit()
    {
        ...
        if (!empty($post_data))
        {
            ...
            if ($this->form_validation->run() != FALSE)
            {
                ...
                if(!empty($card_selected))
                {
                    deleteRecords($this->tbl_orders_cards,array('affiliate_id'=>$edit_id));
                    foreach($card_selected as $card_id)
                    {
                ...
                if(!empty($shipment_selected))
                {
                    deleteRecords($this->tbl_orders_shippings,array('affiliate_id'=>$edit_id));
                    foreach($shipment_selected as $card_type)
                    {
        ...
        $data['cards'] = get_records('tbl_products', array('status' => 1));
        $orders_cards = get_records($this->tbl_orders_cards, array('affiliate_id' => $edit_id));
        $orders_shipping = get_records($this->tbl_orders_shippings, array('affiliate_id' => $edit_id));
        
        $affCardArr = array();
        if(!empty($orders_cards))
        {
            foreach($orders_cards as $row)
            {
                $affCardArr[$row->card_id] = $row->offer_price;
            }
        }
        $data['orders_cards'] = $affCardArr;
        
        $affShippingArr = array();
        if(!empty($orders_cards))
        {
            foreach($orders_shipping as $row)
            {
                $affShippingArr[$row->shipping_type] = $row->price;
            }
        }
        $data['orders_shipping'] = $affShippingArr;
    ...
    public function view()
    {
        ...
        $data['viewdata'] = getDataRecord($this->tbl_name, array('id' => $edit_id));
        /*$data['cards'] = get_records('tbl_products', array('status' => 1));
        $orders_cards = get_records($this->tbl_orders_cards, array('affiliate_id' => $edit_id));
        $orders_shipping = get_records($this->tbl_orders_shippings, array('affiliate_id' => $edit_id));
    ...
    public function delete()
    {
        ...
        deleteRecords($this->tbl_orders_cards,array('affiliate_id'=>$id));
        deleteRecords($this->tbl_orders_shippings,array('affiliate_id'=>$id));
